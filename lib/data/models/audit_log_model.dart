...
class AuditLogModel extends Equatable {
  ...
  factory AuditLogModel.fromJson(Map<String, dynamic> json) {
    ...
    if (userName.isEmpty && json['user'] is Map) {
      userName = (json['user']['name'] ?? json['user']['email'] ?? '').toString();
    }
    if (userName.isEmpty && json['createdBy'] is Map) {
      userName = (json['createdBy']['name'] ?? json['createdBy']['email'] ?? '').toString();
    }
    ...
    String details = (json['details'] ?? json['message'] ?? json['description'] ?? '').toString();
    ...
    if (json['changes'] is Map) {
      ...
      if (from.isEmpty && to.isEmpty) {
        from = (changes['fromStatus'] ?? changes['old'] ?? changes['previous'] ?? '').toString();
        to = (changes['toStatus'] ?? changes['new'] ?? changes['next'] ?? '').toString();
      }
      ...
      if ((from.isEmpty && to.isEmpty) && changes.isNotEmpty) {
        for (final value in changes.values) {
          if (value is Map) {
            ...
            String nestedFrom = (nested['from'] ??
                    nested['fromStatus'] ??
                    nested['old'] ??
                    nested['previous'] ??
                    '')
                .toString();
            String nestedTo = (nested['to'] ??
                    nested['toStatus'] ??
                    nested['new'] ??
                    nested['next'] ??
                    '')
      ...
      if (from.isEmpty && to.isEmpty && changes.isNotEmpty) {
        ...
        changes.forEach((key, value) {
          if (value is Map && (value.containsKey('from') || value.containsKey('to') || value.containsKey('old') || value.containsKey('new'))) {
            final f = (value['from'] ?? value['old'] ?? value['previous'] ?? '').toString();
            final t = (value['to'] ?? value['new'] ?? value['next'] ?? '').toString();
            if (f.isNotEmpty || t.isNotEmpty) {
              changeSummaries.add('$key: ${f.isEmpty ? "None" : f} → ${t.isEmpty ? "None" : t}');
            }
          }
        });
      ...
      if (details.isEmpty && from.isEmpty && to.isEmpty && changes.isNotEmpty && changes.length < 5) {
      ...
      if ((json['changes'] is String || json['changes'] is num || json['changes'] is bool) && !str.startsWith('{')) {
        details = str;
      }
    }
    ...
    return AuditLogModel(
      ...
      action: (json['action'] ?? json['event'] ?? json['type'] ?? '').toString(),
      createdAt: parsed,
      entityType: (json['entityType'] ?? json['entity_type'] ?? json['objectType'] ?? json['resource'] ?? '').toString(),
      entityId: (json['entityId'] ?? json['entity_id'] ?? json['objectId'] ?? json['resourceId'] ?? '').toString(),
      userId: () {
        ...
        if (json['createdBy'] is Map && json['createdBy']['_id'] != null) return json['createdBy']['_id'].toString();
        if (json['user'] is Map && json['user']['_id'] != null) return json['user']['_id'].toString();
        return '';
      }(),
      userName: userName,
