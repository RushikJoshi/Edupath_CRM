...
/// Central Dio client for all API calls. Base URL and auth are set here.
class ApiService {
  ApiService() {
    ...
    // Bypass SSL certificate validation (for expired/self-signed certificates)
      ...
      _dio.interceptors.add(
        InterceptorsWrapper(
          onRequest: (options, handler) {
            ...
            debugPrint('API_LOG: headers=${_sanitizeHeaders(options.headers)}');
            debugPrint('API_LOG: body=${_sanitizeData(options.data)}');
            handler.next(options);
          },
          onResponse: (response, handler) {
            debugPrint(
              'API_LOG: <<< ${response.statusCode} ${response.requestOptions.uri}',
            );
            debugPrint('API_LOG: response=${_sanitizeData(response.data)}');
            handler.next(response);
          },
          onError: (error, handler) {
            debugPrint(
              'API_LOG: xxx ${error.requestOptions.method} ${error.requestOptions.uri}',
            );
            debugPrint('API_LOG: error=${_sanitizeData(error.response?.data)}');
            handler.next(error);
          },
        ),
      );
