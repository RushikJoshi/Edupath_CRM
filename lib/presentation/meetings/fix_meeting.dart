...
void main() async {
  ...
  content = content.replaceAll(r'\${_getMonth(_startDate!.month)}', '\${_getMonth(_startDate!.month)}');
  content = content.replaceAll(r'\${_startDate!.year}', '\${_startDate!.year}');
  content = content.replaceAll(r'\${_getWeekday(_startDate!.weekday)}', '\${_getWeekday(_startDate!.weekday)}');
  content = content.replaceAll(r'\${_startTime!.format(context)}', '\${_startTime!.format(context)}');

  // Fix literal \n in hint strings and text strings
  content = content.replaceAll(r'\n(1h 00m)', '\n(1h 00m)');
  content = content.replaceAll(r'Google Meet\nJoin Meeting', 'Google Meet\nJoin Meeting');
